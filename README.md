...
## Database Setup
Ensure Postgres is run in the backend. Create a database to use, which is to be filled into the .env file. 

## Configuration
Ensure the configuration .env file is filled up. 

## Auth0 Configuration
 - Just change `user_name`, `password` and `port` to whatever you choose while installing postgres.
>_tip_: `user_name` usually defaults to `postgres` and `port` always defaults to `localhost:5432` while installing postgres, most of the time you just need to change the `password. just insert your data 
